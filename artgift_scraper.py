...
def infer_category(url: str, title: str, size_title: str, product_kind: str, cfg: dict[str, Any]) -> int:
    ...
    if product_kind == "bodysuit":
        if re.search(r"момич|girl|принцес|дъщер|красива|госпожиц|сладуран|кралица|фея|бебка", text):
            return 26325
...
def browser_variants(url: str, timeout_ms: int, base_price: float, product_kind: str) -> list[Variant]:
    ...
    def option_names(page: Any, selector: str) -> list[str]:
        # Preserve the storefront order while removing duplicate selector labels.
        return page.locator(selector).evaluate_all(
            "els => [...new Set(els.filter(e => e.offsetParent !== null && !e.classList.contains('disabled'))"
    ...
    with sync_playwright() as p:
        ...
        if product_kind == "bodysuit":
            ...
            # Art-Gift re-renders the variation selectors after a colour click. Reloading
            # the product before every colour prevents later colours from disappearing
            # from the DOM and ensures every offered colour is enumerated.
...
def _variant_key(variant: Variant, product_kind: str) -> tuple[str, str, str]:
    size = normalize_space(variant.size_raw).casefold()
    color = normalize_space(variant.color_raw).casefold()
    sleeve = normalize_space(variant.sleeve_raw).casefold() if product_kind == "bodysuit" else ""
    ...
    *,
        ...
        try:
            ...
            if cfg.get("exact_variants_with_browser"):
                try:
                    ...
                    if exact:
                        if product.product_kind == "tshirt":
                            static_colors = distinct_variant_values(static, "color_raw")
                            exact_colors = distinct_variant_values(exact, "color_raw")
                            if len(exact_colors) < len(static_colors):
                                logging.warning(
                                    "Browser returned only %d/%d colours for %s; preserving the complete static matrix",
                                    len(exact_colors), len(static_colors), url,
                                )
    ...
    report = {
        ...
        "warnings": [
            ...
            "T-shirt browser results are checked against the static colour/size matrix so a dynamic selector refresh cannot silently remove colours.",
            "Bodysuit titles with clear feminine terms are mapped to category 26325; unisex bodysuits default to category 26402.",
